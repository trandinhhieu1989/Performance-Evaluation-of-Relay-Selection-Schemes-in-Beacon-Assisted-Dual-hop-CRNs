...
%
OP1             = zeros(1,length(xR));
OP2             = zeros(1,length(xR));
for aa = 1 : length(xR)
    [OP1(aa), OP2(aa)] = ham(PBdB,Muy,MM,NN,KK,xR(aa),xB,yB,xP,yP,Eta,AP,PL,Cth,tSS,tSP);
end
%OP1
%OP2
OP              = min(OP1,OP2);
semilogy(xR,OP1,'r--'); grid on;hold on;
semilogy(xR,OP2,'r--'); grid on;hold on;
