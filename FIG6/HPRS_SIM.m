...
%
PP             = 10.^(PBdB/10);
OP1            = zeros(1,length(tSS));
OP2            = zeros(1,length(tSS));
%
for aa = 1 : length(tSS)
    ...
    for bitnum   =  1 : Num_T
        ...
        TS         = II/CG_SP_max/(1+tSP(aa));
        %
        PowS       = min(ES,TS);
        SNR_SR     = PowS*CG_SR_max/(1 + tSS(aa)*PowS*CG_SR_max);
        %
        ER         = 0;
        for bb = 1 : KK
        ...
        TR         = II/CG_RP_max/(1+tSP(aa));
        %
        PowR       = min(ER,TR);
        h_RD       = sqrt(1/LRD/2)*(randn(1,1)+1i*randn(1,1));
        SNR_RD     = PowR*abs(h_RD)^2/(1 + tSS(aa)*PowR*abs(h_RD)^2);
        %
        ...
        TS2        = II/CG_SP_max2/(1+tSP(aa));
        %
        PowS2      = min(ES2,TS2);
        SNR_SR2    = PowS2*abs(h_SR2)^2/(1 + tSS(aa)*PowS2*abs(h_SR2)^2);
        %
        CG_RD_max  = 0;
        for bb = 1 : MM
        ...
        %                
        TR2        = II/CG_RP_max2/(1+tSP(aa));
        PowR2      = min(ER2,TR2);        
        SNR_RD2    = PowR2*CG_RD_max/(1 + tSS(aa)*PowR2*CG_RD_max);
        %
