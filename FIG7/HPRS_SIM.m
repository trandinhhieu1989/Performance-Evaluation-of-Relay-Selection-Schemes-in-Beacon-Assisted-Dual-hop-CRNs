...
%
PP             = 10.^(PBdB/10);
OP1            = zeros(1,length(AP));
OP2            = zeros(1,length(AP));
%
for aa = 1 : length(AP)
    fprintf('Running %d per %d \n',aa,length(AP));
    for bitnum   =  1 : Num_T
        II         = Muy*PP;
        kap        = 2*Eta*AP(aa)/(1-AP(aa));
        % Parameters
        LSR        = xR^PL;
        LRD        = (1-xR)^PL;
        ...
        %
        Capa       = (1-AP(aa))/2*log2(1 + min(SNR_SR,SNR_RD));     
        %
        if (Capa   < Cth)
            OP1(aa) = OP1(aa) + 1;
        end   
        ...
        %
        Capa2      = (1-AP(aa))/2*log2(1 + min(SNR_SR2,SNR_RD2));     
        %        
        if (Capa2  < Cth)
            OP2(aa) = OP2(aa) + 1;
        end                           
